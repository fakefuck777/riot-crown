...
test('搜尋頁與商品頁可開且為 200', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /void chain/i })).toBeVisible({ timeout: 15_000 });
});
...
test('首頁加購 → 開購物車 → 關閉（桌面）', async ({ page }) => {
  ...
  const acquire = page.getByRole('button', { name: /ACQUIRE/i }).first();
  await acquire.scrollIntoViewIfNeeded();
  await acquire.click({ timeout: 15_000 });

  // addToCart() 會同步開啟抽屜；此時再點導覽列購物車會被抽屜擋住指標
  const dialog = page.getByRole('dialog', { name: /YOUR ARTIFACTS/i });
